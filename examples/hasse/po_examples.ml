...
struct
  ...
  let rec compare l1 l2 acc =
    match (l1, l2) with
    | h1 :: t1, h2 :: t2 ->
        if h1 < h2 then if acc = Greater then Unknown else compare t1 t2 Lower
        else if h1 > h2 then
          if acc = Lower then Unknown else compare t1 t2 Greater
  ...
  let rand_el () =
    let l = ref [] in
    for _i = 1 to Spec.len do
      l := Random.int Spec.choices :: !l
    done;
    !l
  ...
  let pretty_print ppf l =
    let rec aux ppf = function
      | [] -> ()
      | [ x ] -> fprintf ppf "%d" x
      | h :: t -> fprintf ppf "%d, %a" h aux t
    in
    fprintf ppf "[%a]" aux l
end

module MakePOBList (Spec : sig
  val len : int
end) =
struct
  type el = bool list
  ...
  let rec compare_loop l1 l2 acc =
    ...
    | h1 :: t1, h2 :: t2 ->
        if h1 then
          ...
          if acc = Greater then Unknown else compare_loop t1 t2 Lower
  ...
  let pretty_print ppf l =
    let rec loop ppf = function
      ...
      | h :: t -> fprintf ppf "%d, %a" (int_of_bool h) loop t
    in
