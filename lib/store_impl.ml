...
module Make (Ix : INDEX) = struct
  ...
  let update ix el (gen, m) = (Ix.remove_ix gen ix, Ix.Map.add ix el m)
  ...
  let partition p (gen, m) =
    let coll ix el (yes, no) =
      if p ix el then (yes, Ix.Map.remove ix no) else (Ix.Map.remove ix yes, no)
    in
